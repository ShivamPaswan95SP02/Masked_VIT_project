...
batch_idx = st.sidebar.slider("Batch Index", 0, len(train_loader)-1, 0)
sample_indices = st.sidebar.multiselect("Sample Indices", list(range(batch_size)), [0, 1, 2, 3, 4])  # manualy 
...
def visualize_patching_masking(batch_idx, sample_indices, mask_ratio=0.75):
    """Visualize original images, masked images, and images reconstructed from patches"""
    ...
    st.write("Masked Images (75% patches masked):")
    ...
    patch_recon = rearrange(patches[:num_samples], 'b (h w) (p1 p2 c) -> b c (h p1) (w p2)',
                           h=image_size//patch_size, p1=patch_size, p2=patch_size)
    st.write("Images Reconstructed from Patches:")
...
if __name__ == "__main__":
    ...
    visualize_patching_masking(batch_idx, sample_indices, mask_ratio=0.75)
    ...
    visualize_reconstruction(model, batch_idx, sample_indices)
